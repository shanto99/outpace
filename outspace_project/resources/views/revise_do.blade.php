        ...
        <div class="layout-content-body">
            <div class="title-bar">
                <h1 class="title-bar-title">
                    <span class="d-ib">Revise DO</span>
                </h1>
            ...
            <div class="row">
                <div class="col-sm-8 col-sm-offset-3 col-md-4 col-md-offset-4">
                    <div class="demo-form-wrapper">
                        <label class="control-label" for="select_a_do">Select a DO</label>
                        <div>
                            <select class = "md-form-control" id="select_a_do" name = "do_id">
                                <option disabled selected value> -- select an option -- </option>
                                <option value = "add_bank">Add New Bank</option>
                                @foreach($all_do as $do)
                                    <option value="{{ $do->order_no }}">{{ $do->order_no }}</option>
                                @endforeach
                            </select>
                            <label class="md-control-label"></label>
                        </div>
                        <div class="md-form-group">
                            <label for="lc_id">LC ID</label>
                            <input id="lc_id" class="md-form-control" type="text" value="">
                            <label class="md-control-label"></label>
                        </div>
                        <div class="md-form-group">
                            <label for="lc_ammend_no">Ammendment No</label>
                            <input id="lc_ammend_no" class="md-form-control" type="text" value="">
                            <label class="md-control-label"></label>
                        </div>
                        <div class="md-form-group">
                            <label for="approved_by">Do Approved By</label>
                            <input id="approved_by" class="md-form-control" type="text" value="" required>
                            <label class="md-control-label"></label>
                        </div>
                        <div class="md-form-group">
                            <label for="payment_period">Notes</label>
                        ...
                        <button class="btn btn-primary btn-block" type="submit" id="save_do">Save & Confirm</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $( function() {
        ...
        $("#select_a_do").change(function(){
            var e = document.getElementById("select_a_do");
            var do_id = e.options[e.selectedIndex].value;
            $.ajax({
                type: "GET",
                cache: false,
                url: "{{ route('get_do_detail') }}",
                data: {do_id: do_id},
                success: function(data){
                    document.getElementById('lc_id').value = data.lc_id;
                    document.getElementById('lc_ammend_no').value = data.lc_ammend_no;
                    document.getElementById('approved_by').value = data.do_finally_approve;
                },
                error: function () {
                    alert("Error");
                }
    ...
    $("#save_do").click(function () {
        ...
        var TableData = [];
        var do_id = document.getElementById('select_a_do').value;
        $('#PITable tr').each(function (row, tr) {
            ...
            if (row > 0) {
                ...
                TableData[rowIndex] = {
                    ...
                    , "requested": $(tr).find("td:eq(5) input[type='text']").val()
        ...
        $.ajax({
            type: "GET",
            cache: false,
            url: "{{ route('save_do_detail') }}",
            data: {products: TableData, do_id: do_id},
            success: function (data) {
                alert(data);
            }
        });
